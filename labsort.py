...
def initialize_state():
    labs = [
        "吉川研究室（データ価値創造基盤）", "小山田研究室（空気とAIの快適空間）", "原研究室（イノベーション・マネジメント）",
        "鎌原研究室（マルチメディア）", "笠原研究室（パターン認識）", "杉山研究室（コンテンツ解析学）",
...
if st.session_state.phase == "compare":
    ...
    if st.session_state.current_index < len(st.session_state.current_pairs):
        ...
        with col1:
            if st.button(f"🅰 {lab1.split('（')[0]}\n　{lab1}", key=f"btn1_{st.session_state.current_index}"):
                process_selection(a, b)
        with col2:
            if st.button(f"🅱 {lab2.split('（')[0]}\n　{lab2}", key=f"btn2_{st.session_state.current_index}"):
